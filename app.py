...
def get_db_connection(db_name):
    """Establishes and caches a database connection."""
    # Add check_same_thread=False to allow cross-thread usage for Streamlit
    conn = sqlite3.connect(db_name, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    return conn

def get_order_details(order_id):
    conn = get_db_connection(DB_NAME) # <--- Add DB_NAME here
    cursor = conn.cursor()
    cursor.execute("SELECT order_id, items, status, customer_id FROM orders WHERE order_id = ?", (order_id,))
    order = cursor.fetchone()
    if order:
        return {'Order_ID': order[0], 'Items': order[1], 'Status': order[2], 'Customer_ID': order[3]}
    return None
...
# --- Customer and Points Database Functions ---
def get_customer_by_mobile(mobile_number):
    conn = get_db_connection(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT customer_id, mobile_number, email, points FROM customers WHERE mobile_number = ?", (mobile_number,))
    customer = cursor.fetchone()
    # conn.close()
    if customer:
        return {'customer_id': customer[0], 'mobile_number': customer[1], 'email': customer[2], 'points': customer[3]}
    return None

def add_customer(mobile_number, email=None):
    conn = get_db_connection(DB_NAME)
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO customers (mobile_number, email) VALUES (?, ?)", (mobile_number, email))
        conn.commit()
        return cursor.lastrowid # Return the new customer_id
    except sqlite3.IntegrityError:
        # This means mobile_number or email (if provided) is not unique
        return None
    finally:
        # conn.close()
        pass
...
def update_customer_points(customer_id, points_to_add):
    conn = get_db_connection(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("UPDATE customers SET points = points + ? WHERE customer_id = ?", (points_to_add, customer_id))
    conn.commit()
    # conn.close()
...
# Dummy function to simulate a reward redemption
def redeem_points(customer_id, points_cost, reward_name):
    current_points = get_customer_points(customer_id)
    if current_points >= points_cost:
        conn = get_db_connection(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("UPDATE customers SET points = points - ? WHERE customer_id = ?", (points_cost, customer_id))
        conn.commit()
        # conn.close()
        # In a real app, this would also generate a unique voucher code, save to a 'redeemed_vouchers' table, etc.
        return f"Voucher for {reward_name}: BK-{customer_id}-{random.randint(1000, 9999)}"
    return None
...
def initialize_game_state():
    ...
    st.session_state.guess_number_active = False
...
def initialize_customer_state():
    """Initializes or resets customer-related states."""
    st.session_state.logged_in_customer_id = None
    st.session_state.logged_in_mobile = None
    st.session_state.show_loyalty_dashboard = False # New state to show loyalty dashboard
...
def reset_all_states():
    ...
    initialize_burger_stack_game_state()
    initialize_customer_state() # Reset customer state
    st.session_state.mini_game_menu_active = False

# Ensure session state is initialized for all features
if 'quiz_active' not in st.session_state: # Check for a base state to initialize all
    reset_all_states() # Initialize all states on first run
    # Explicitly ensure these are off initially if not already in reset_all_states
    st.session_state.guess_number_active = False
    st.session_state.burger_stack_active = False
    st.session_state.mini_game_menu_active = False
    st.session_state.show_loyalty_dashboard = False # Ensure this is off
...
# Main content area - only show order details if no quiz or game/loyalty dashboard is active
if not st.session_state.quiz_active and not st.session_state.quiz_completed and \
   ...
   not st.session_state.mini_game_menu_active and not st.session_state.show_loyalty_dashboard: # Added show_loyalty_dashboard
    # --- Order ID Input and Customer Login ---
    st.header("Welcome to Burger King! 👑")

    # Customer Login/Registration Section
    with st.expander("🍔 Loyalty Program: Log in or Join!"):
        if st.session_state.logged_in_customer_id:
            customer_data = get_customer_by_mobile(st.session_state.logged_in_mobile)
            if customer_data:
                st.success(f"Welcome back, Customer (ID: {customer_data['customer_id']})! You have **{customer_data['points']}** loyalty points. 🎉")
            else:
                st.error("Error retrieving customer data.")
            if st.button("Logout", key="logout_btn"):
                initialize_customer_state() # Reset customer state
                st.rerun()
        else:
            st.write("Enter your mobile number to earn points and redeem rewards!")
            col_mobile, col_email = st.columns([0.6, 0.4])
            with col_mobile:
                mobile_input = st.text_input("Mobile Number (10 digits)", max_chars=10, key="mobile_loyalty_input")
            with col_email:
                email_input = st.text_input("Email (Optional)", key="email_loyalty_input")

            if st.button("Login / Register", key="login_register_btn"):
                if mobile_input and len(mobile_input) == 10 and mobile_input.isdigit():
                    customer = get_customer_by_mobile(mobile_input)
                    if customer:
                        st.session_state.logged_in_customer_id = customer['customer_id']
                        st.session_state.logged_in_mobile = customer['mobile_number']
                        st.success(f"Logged in successfully! Welcome back, you have **{customer['points']}** points.")
                        st.rerun()
                    else:
                        # Register new customer
                        new_customer_id = add_customer(mobile_input, email_input if email_input else None)
                        if new_customer_id:
                            st.session_state.logged_in_customer_id = new_customer_id
                            st.session_state.logged_in_mobile = mobile_input
                            st.success(f"Registered and logged in! Welcome! You now have 0 points. Start playing games to earn!")
                            st.rerun()
                        else:
                            st.error("Could not register. Mobile number or email might already be in use.")
                else:
    ...
    st.write("Enter your Order ID to see details:")
    ...
    if order_id_input:
        ...
        if order_details:
            ...
            order_customer_id = order_details['Customer_ID'] # Get customer_id from order details

            st.success("Order Found! 🎉")
            st.write(f"**Items:** {items}")
            st.write(f"**Status:** {status}")

            # Display linked customer info if available
            if order_customer_id:
                conn = get_db_connection(DB_NAME)
                cursor = conn.cursor()
                cursor.execute("SELECT mobile_number FROM customers WHERE customer_id = ?", (order_customer_id,))
                linked_mobile = cursor.fetchone()
                # conn.close()
                if linked_mobile:
                    st.info(f"This order is linked to mobile number: **{linked_mobile[0]}**")
                else:
                    st.info("This order is linked to an unknown customer.")
            else:
                st.info("This is an **anonymous order** (not linked to a loyalty customer).")
            ...
            col1, col2, col3, col4 = st.columns(4) # Added a column for Loyalty
            ...
            with col2:
                if st.button("🧠 Play a Quiz related to your order"):
                    ...
                    # ... (quiz topic generation logic remains the same) ...
            ...
            with col3:
                if st.button("🎮 Play a Short Game"):
                    ...
                    st.rerun()
            with col4: # New column for Loyalty Dashboard
                if st.button("💰 My Points & Rewards"):
                    if st.session_state.logged_in_customer_id:
                        reset_all_states()
                        st.session_state.show_loyalty_dashboard = True
                        st.rerun()
                    else:
                        st.warning("Please log in with your mobile number to view your points and rewards.")

        else:
            st.warning(f"Order ID `{order_id_input}` not found. Please double-check and try again.")
            st.info("Currently, our system recognizes Order IDs: 38, 39, 40, 41, 42.")

# --- Game Selection Menu (This block remains the same) ---
...
# --- Quiz Display Logic (This block remains the same) ---
...
# --- Quiz Completed Logic (This block remains the same) ---
    ...
    if st.session_state.score == len(st.session_state.questions):
        ...
        # --- AWARD POINTS FOR QUIZ WIN ---
        if st.session_state.logged_in_customer_id:
            points_earned = 20 # Example points
            update_customer_points(st.session_state.logged_in_customer_id, points_earned)
            st.success(f"You earned {points_earned} loyalty points for completing the quiz! Check your 'My Points & Rewards' section.")
        # --- END AWARD POINTS ---
...
# --- Guess the Number Game Logic (Modified for points) ---
elif st.session_state.guess_number_active:
    ...
    if not st.session_state.game_over:
        ...
        if st.button("Submit Guess", type="primary"):
            ...
            if user_guess < st.session_state.secret_number:
                ...
                st.balloons()
                # --- AWARD POINTS FOR GAME WIN ---
                if st.session_state.logged_in_customer_id:
                    points_earned = 15 # Example points for this game
                    update_customer_points(st.session_state.logged_in_customer_id, points_earned)
                    st.success(f"You earned {points_earned} loyalty points for winning! Check your 'My Points & Rewards' section.")
                # --- END AWARD POINTS ---
            st.rerun()
    else: # Game is over
...
# --- Burger Stack Game Logic (Modified for points) ---
    ...
    st.markdown(st.session_state.burger_game_feedback)

    if st.session_state.burger_game_status == "playing":
        st.subheader("Available Ingredients:")
        cols = st.columns(len(WHOOPER_RECIPE))
                ...
                if st.button(f"{ingredient['emoji']} {ingredient['name']}", key=f"ingredient_btn_{ingredient['name']}"):
                    ...
                    if ingredient['name'] == expected_ingredient['name']:
                        ...
                        if st.session_state.next_ingredient_index == len(WHOOPER_RECIPE):
                            ...
                            # --- AWARD POINTS FOR GAME WIN ---
                            if st.session_state.logged_in_customer_id:
                                points_earned = 25 # Example points for this game
                                update_customer_points(st.session_state.logged_in_customer_id, points_earned)
                                st.success(f"You earned {points_earned} loyalty points for winning! Check your 'My Points & Rewards' section.")
                            # --- END AWARD POINTS ---
                    else:
...
# --- Loyalty Dashboard (NEW BLOCK) ---
elif st.session_state.show_loyalty_dashboard:
    st.header("💰 My Points & Rewards")
    if st.session_state.logged_in_customer_id:
        current_points = get_customer_points(st.session_state.logged_in_customer_id)
        st.subheader(f"You currently have: **{current_points}** loyalty points! ✨")

        st.markdown("---")
        st.subheader("Redeem Your Points:")

        rewards = [
            {"name": "Free Small Fries", "cost": 50, "image": "🍟"},
            {"name": "Free Soft Drink", "cost": 75, "image": "🥤"},
            {"name": "10% Off Next Order", "cost": 100, "image": "🛍️"},
            {"name": "Free Whopper Jr.", "cost": 150, "image": "🍔"},
        ]

        # Display rewards in columns
        reward_cols = st.columns(len(rewards))
        for i, reward in enumerate(rewards):
            with reward_cols[i]:
                st.markdown(f"#### {reward['image']} {reward['name']}")
                st.write(f"Cost: **{reward['cost']}** points")
                if current_points >= reward['cost']:
                    if st.button(f"Redeem {reward['name']}", key=f"redeem_btn_{reward['name']}"):
                        redeem_code = redeem_points(st.session_state.logged_in_customer_id, reward['cost'], reward['name'])
                        if redeem_code:
                            st.success(f"Successfully redeemed! Here's your code: `{redeem_code}`. Show this to the cashier to claim your reward!")
                            st.session_state.show_loyalty_dashboard = True # Keep showing dashboard
                            st.rerun() # Rerun to update points display
                        else:
                            st.error("Something went wrong with redemption. Please try again.")
                else:
                    st.button(f"Need {reward['cost'] - current_points} more points", disabled=True, key=f"need_more_btn_{reward['name']}")

        st.markdown("---")
        st.subheader("Your Order History (Linked to this Account):")
        conn = get_db_connection(DB_NAME)
        cursor = conn.cursor()
        cursor.execute("SELECT order_id, items, status FROM orders WHERE customer_id = ? ORDER BY order_id DESC", (st.session_state.logged_in_customer_id,))
        customer_orders = cursor.fetchall()
        # conn.close()

        if customer_orders:
            for order in customer_orders:
                st.write(f"**Order ID:** {order[0]} | **Items:** {order[1]} | **Status:** {order[2]}")
        else:
            st.info("No orders found linked to this account yet. Place an order and link it to your mobile number to see history here!")

    else:
        st.warning("Please log in with your mobile number to view your points and rewards.")

    if st.button("🏠 Return to Order Details", key="loyalty_return_btn"):
        reset_all_states()
        st.rerun()
